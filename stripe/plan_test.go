...
func TestCreatePlan(t *testing.T) {
	...
	} else if strings.HasPrefix(err.Error(), "Invalid currency: "+strings.ToLower(p3.Currency)) == false {
		t.Errorf("Expected %s, got %s", "Invalid currency: "+strings.ToLower(p3.Currency), err.Error())
	}
}
...
func TestListPlan(t *testing.T) {
	...
	// there are also 2 there were added manually.
	if len(plans) != 4 {
