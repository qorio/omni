...
func (this *ShortyEndPoint) ReportInstallHandler(resp http.ResponseWriter, req *http.Request) {
	...
	cannotDetermineContent := false

	// a unique user identifier -- generated by us and the lastViewed short code
	userId, lastViewed := "", ""
	if readCookieError := secureCookie.ReadCookie(req, "uuid", &userId); readCookieError != nil {
		glog.Infoln("Cannot read cookie uuid", readCookieError)
		cannotDetermineContent = true
	} else if readCookieError = secureCookie.ReadCookie(req, "last", &lastViewed); readCookieError != nil {
		glog.Infoln("Cannot read cookie last", readCookieError)
		cannotDetermineContent = true
	} else {
		cannotDetermineContent = userId == "" || lastViewed == ""
	}
